...
func _process(_delta: float) -> void:
	if brighten:
		timer += 1
		$BrightenCanvasLayer/ColorRect.material.set_shader_parameter("colourAdd", (timer as float) / 300)
		$BrightenCanvasLayer/ColorRect.material.set_shader_parameter("multiplier", 1.0 + (timer as float) / 60)
		if timer >= 300:
			get_tree().change_scene_to_file("res://Scenes/RootScenes/level_1.tscn")